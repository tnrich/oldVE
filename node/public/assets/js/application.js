...
require(['domready!','core','plugins','utils','de-utils'], function () {
  ...
  var J5 = Class.extend({
    ...
    var Resources = Class.extend({
        init: function(){
            var _self = this;
            $.getJSON('/assets/data/icons.json', function(data) {
              _self.icons = data;
              _self.renderIcons();
            })
        },
    ...
    var Inspector = Class.extend({
        ...
        show: function(){$(this.body).slideDown();},
        hide: function(){$(this.body).slideUp();},
        toggle: function(){
            if($('#inspector .inspector-body').is(':visible')) inspector.hide();
            else inspector.show();
        },
        bindUI: function(){

...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    if (input.parts) {
      ...
      message = arguments;
        ...
        //console.log('targetNSAlias: '+input.targetNSAlias);
        //console.log('xmlns: '+require('util').inspect(input.xmlns,true,2,true));
        
        if(method.$name == 'createPlasmid')
        {

            input.targetNSAlias = 'ns2';
            input.$type = 'ns2:createPlasmid';
            input.namespace = 'ns2';
            console.log(input);
        }
    ...
    xml = "<soap:Envelope " + 
        ...
        if(xml.match(/ns2/)) 
        {
            xml = xml.replace(/(ns2:(\w|\w)+)>/, xml.match(/(ns2:(\w|\w)+)>/)[1] + ' xmlns:ns2="https://api.registry.jbei.org/">')
            xml = xml.replace('xmlns:tns="https://api.registry.jbei.org/"','');
            console.log(xml);
        }

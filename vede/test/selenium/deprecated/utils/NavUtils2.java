...
public class NavUtils2 {
	...
	public static void logIn(WebDriver driver, String username, String password) {
		...
		SelUtil.findElement(driver, By.id("auth-login-btn-btnIconEl")).click();
	}
	...
	public static void toggleExpansionOfProject(WebDriver driver, String name, boolean all) {
		...
		for (WebElement w : items) {
			if(SelUtil.findElement(w, By.xpath("../span")).getAttribute("innerHTML").equals(name)) {			
				SelUtil.click(w);
				if(!all) return;
			}
		}
	...
	public static void createNewProject(WebDriver driver, String name) {
		SelUtil.findElement(driver, By.xpath("//div[@id='ProjectPanel']//tr")).click();	
		
		WebElement window = SelUtil.findElement(driver, By.cssSelector(".x-window.x-message-box.x-layer"));
		
		SelUtil.findElement(window, By.xpath("//input")).clear();
		SelUtil.findElement(window, By.xpath("//input")).sendKeys(name);
		
		List<WebElement> items = window.findElements(By.cssSelector(".x-btn-button"));
		for (WebElement w : items) {
			if(SelUtil.findElement(w, By.xpath("./span")).getAttribute("innerHTML").equals("OK")) {
				SelUtil.findElement(w, By.xpath("./span[@role='img']")).click();
			}
		}
	}
	...
	public static void createNewSequence(WebDriver driver, String sequenceName, String projectName) {		
		List<WebElement> projectPanelItems = SelUtil.findElements(driver, By.xpath("//div[@id='ProjectPanel']//tbody//div"));
		List<WebElement> projectPanelProjects = SelUtil.findElements(driver, By.xpath("//div[@id='ProjectPanel']//tbody//div/img[@class=' x-tree-elbow-img x-tree-elbow-plus x-tree-expander']"));
		projectPanelProjects.addAll(SelUtil.findElements(driver, By.xpath("//div[@id='ProjectPanel']//tbody//div/img[@class=' x-tree-elbow-img x-tree-elbow-end-plus x-tree-expander']")));
		...
		for (WebElement w : projectPanelProjects) {
			if(SelUtil.findElement(w, By.xpath("../span")).getAttribute("innerHTML").equals(projectName)) {			
				int wIdx = projectPanelItems.indexOf(SelUtil.findElement(w, By.xpath("..")));
				if (wIdx!=-1) {
					idx = wIdx;
					SelUtil.click(w);			
					break;
		...
		if(newProjectPanelLength<oldProjectPanelLength) NavUtils2.toggleExpansionOfProject(driver, projectName, false);
		
		
		SelUtil.findElements(driver, By.xpath("//div[@id='ProjectPanel']//tbody//div")).get(idx+2).click();
		
		WebElement window = driver.findElement(By.cssSelector(".x-window.x-message-box.x-layer"));
		SelUtil.findElement(window, By.xpath("//input")).clear();
		SelUtil.findElement(window, By.xpath("//input")).sendKeys(sequenceName);
		
		List<WebElement> items = SelUtil.findElements(window, By.cssSelector(".x-btn-button"));
		for (WebElement w : items) {
			if(SelUtil.findElement(w, By.xpath("./span")).getAttribute("innerHTML").equals("OK")) {
				SelUtil.findElement(w, By.xpath("./span[@role='img']")).click();
			}
		...
		if (driver.findElement(By.cssSelector(".x-window.x-message-box.x-layer")).getText().contains(NavUtils2.NEW_SEQUENCE_ALREADY_EXISTS_TXT)) {
			window = driver.findElement(By.cssSelector(".x-window.x-message-box.x-layer"));
			items = SelUtil.findElements(window, By.cssSelector(".x-btn-button"));
			for (WebElement w : items) {
				if(SelUtil.findElement(w, By.xpath("./span")).getAttribute("innerHTML").equals("Cancel")) {
					SelUtil.findElement(w, By.xpath("./span[@role='img']")).click();
				}
			}
		}
	...
	public static void createNewDesign(WebDriver driver, String designName, String projectName) {		
		...
		if (driver.findElement(By.cssSelector(".x-window.x-message-box.x-layer")).getText().contains(NavUtils2.NEW_DESIGN_ALREADY_EXISTS_TXT)) {
			window = driver.findElement(By.cssSelector(".x-window.x-message-box.x-layer"));

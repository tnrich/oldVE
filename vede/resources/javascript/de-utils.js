    ...
    */
    ...
    function validateSequence(sequence)
    {
        if(sequence["de:format"]=="FASTA")
        {
            ...
            if(data)
            {   
                var sqs = sanitizeInput(sequence["de:content"]).match(/([GCAT]+)/g);
                console.log(sqs);
                if(sqs.length>2)
                {
                    console.log("More than one sequence");
                    $('#selectSequence-tablebody').html("");
                    $(sqs).each(function(key,value){
                        console.log(value);
                        if(key!=0) $('#selectSequence-tablebody').append("<tr><td>"+value+"</td></tr>");
                    });
                    
                    $('#selectSequence-tablebody tr').one("click",function(){
                        $('#selectSequence').modal('hide');
                    });
                    
                    $('#selectSequence').modal();
                }
                return true;
            }
            else
